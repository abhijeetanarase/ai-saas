...
export default function TemplateCreatePage() {
  ...
  const handleStyleChange = (field: string, value: any) => {
    setForm(f => ({ ...f, style: { ...f.style, [field]: value } }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSuccess(false);
    try {
      await apiService.post('/templates', form);
      setSuccess(true);
      setForm({ name: '', description: '', style: { ...DEFAULT_STYLE }, previewImage: '' });
    } catch (err: any) {
    ...
    <div className="max-w-5xl mx-auto py-10 px-2">
      <div className="rounded-2xl border border-gradient-to-r from-blue-200 via-purple-200 to-pink-200 dark:from-gray-700 dark:via-gray-700 dark:to-gray-800 bg-white/80 dark:bg-gray-900/80 shadow-2xl backdrop-blur-md p-8 space-y-8 animate-fadein">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight drop-shadow-sm">Create New Blog Template</h1>
        </div>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
            ...
            <Button type="submit" className="mt-4 w-full" isLoading={submitting}>
              {submitting ? 'Creating...' : 'Create Template'}
            </Button>
            {success && <div className="text-green-600 dark:text-green-400 mt-2">Template created successfully!</div>}
